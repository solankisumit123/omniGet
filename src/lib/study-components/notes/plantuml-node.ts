...
declare module "@tiptap/core" {
  interface Commands<ReturnType> {
    plantumlBlock: {
      setPlantuml: (source?: string) => ReturnType;
    };
  }
}
...
const FENCE_OPEN = /^```plantuml\s*$/;
const FENCE_CLOSE = /^```\s*$/;

function plantumlFenceRule(
  state: MarkdownItState,
  startLine: number,
  endLine: number,
  silent: boolean,
...
export const PlantumlBlock = Node.create({
  ...
  addAttributes() {
    return {
      source: {
        ...
        renderHTML: (attrs) => ({
          "data-plantuml-source": String(attrs.source ?? ""),
        }),
      },
    };
  },
  ...
  addNodeView() {
    return ({ node, editor, getPos }) => {
      ...
      dom.className = "tiptap-plantuml-block";
      dom.setAttribute("data-plantuml", "");
      dom.setAttribute("contenteditable", "false");

      let lastSource = String(node.attrs.source ?? "");
      const blockId = Math.random().toString(36).slice(2, 10);

      const view = mount(PlantumlView as unknown as Component, {
        target: dom,
        props: {
          source: lastSource,
          blockId,
      ...
      markdown: {
        ...
        parse: {
          setup(md: MarkdownItInstance) {
            md.block.ruler.before("fence", "plantuml_fence", plantumlFenceRule, {
