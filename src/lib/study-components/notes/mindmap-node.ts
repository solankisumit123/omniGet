...
declare module "@tiptap/core" {
  interface Commands<ReturnType> {
    mindmapBlock: {
      setMindmap: (source?: string) => ReturnType;
    };
  }
}
...
const FENCE_OPEN = /^```mindmap\s*$/;
const FENCE_CLOSE = /^```\s*$/;

function mindmapFenceRule(
  state: MarkdownItState,
  startLine: number,
  endLine: number,
  silent: boolean,
  ...
  const token = state.push("mindmap", "div", 0);
  token.block = true;
  token.markup = "```";
  token.info = "mindmap";
...
export const MindmapBlock = Node.create({
  ...
  addAttributes() {
    return {
      source: {
        default: "",
        parseHTML: (el) => el.getAttribute("data-mindmap-source") ?? "",
        renderHTML: (attrs) => ({
          "data-mindmap-source": String(attrs.source ?? ""),
        }),
      },
    };
  },
  ...
  addNodeView() {
    return ({ node, editor, getPos }) => {
      ...
      dom.className = "tiptap-mindmap-block";
      dom.setAttribute("data-mindmap", "");
      dom.setAttribute("contenteditable", "false");

      let lastSource = String(node.attrs.source ?? "");
      const blockId = Math.random().toString(36).slice(2, 10);

      const view = mount(MindmapView as unknown as Component, {
        target: dom,
        props: {
          source: lastSource,
          blockId,
      ...
      markdown: {
        ...
        parse: {
          setup(md: MarkdownItInstance) {
            md.block.ruler.before("fence", "mindmap_fence", mindmapFenceRule, {
