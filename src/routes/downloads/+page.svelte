<script lang="ts">
  ...
  import OmniTurboHud from "$components/download/OmniTurboHud.svelte";
  ...
  <div class="download-item" data-status={item.status}>
    ...
    {#if item.status === "downloading"}
      ...
      {:else}
        ...
        {#if item.queueKind === "video" || item.platform !== "magnet"}
          <div style="margin-top: 6px;">
            <OmniTurboHud compact={true} downloading={true} speedBps={item.speed} />
          </div>
        {/if}
      {/if}
    {:else if item.status === "seeding"}
